...
const DEFAULT_RESULT_FILENAME = "ai-anime-filter.jpg";
...
const animeExamples = [
  {
    id: 1,
    src: "/assets/anime-filter/1.png",
    alt: "Anime character conversion",
  },
  {
    id: 2,
    src: "/assets/anime-filter/2.png",
    alt: "Anime portrait conversion",
  },
  {
    id: 3,
    src: "/assets/anime-filter/3.png",
    alt: "Anime environment conversion",
  },
];

const AIAnimeFilterPage = () => {
  ...
  const handleConvert = async () => {
    ...
    if (!uploadedImage) {
      toast({
        ...
        description: "Upload a selfie or portrait to anime-fy it.",
        status: "warning",
        duration: 3000,
        isClosable: true,
        position: "top-right",
    ...
    try {
      ...
      const response = await fetch("/api/ai-photo-filter/ai-anime-filter", {
        method: "POST",
        headers: {
      ...
      if (!response.ok) {
        let message = "The anime filter service returned an unexpected error.";
        try {
      ...
      if (!data?.resultUrl) {
        throw new Error("The anime filter service did not return a result URL.");
      }
      ...
      toast({
        title: "Anime filter applied!",
        description: "Download your anime-styled portrait.",
        status: "success",
        duration: 3000,
        isClosable: true,
        position: "top-right",
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error("Unable to download the anime-styled portrait.");
      }
      ...
      toast({
        ...
        description: "We opened the anime-styled portrait in a new tab so you can download it manually.",
        status: "info",
        duration: 3000,
        isClosable: true,
        position: "top-right",
  ...
  const handleSaveToLibrary = async () => {
    ...
    try {
      const response = await fetch("/api/media", {
        ...
        body: JSON.stringify({
          url: resultUrl,
          prompt: "AI Anime Filter - Vibrant anime-style illustration",
          source: "ai-anime-filter",
        }),
      });

      if (!response.ok) {
        let message = "Failed to save portrait to your library.";
        try {
          const errorPayload = await response.json();
          if (errorPayload?.error) {
            message = errorPayload.error;
      ...
      toast({
        ...
        description:
          error instanceof Error
            ? error.message
            : "Unable to save this portrait. Please try again.",
        status: "error",
        duration: 4000,
        isClosable: true,
        position: "top-right",
            ...
            >
              ...
              <Box maxW={{ base: "100%", md: "800px" }}>
                <Text
                  ...
                  opacity={0.9}
                >
                  Turn selfies and photos into vibrant anime-style illustrations—perfect for avatars, posters, and social content.
                </Text>
              </Box>
            </MotionVStack>
              ...
              >
                    ...
                    >
                      ...
                      >
                        ...
                        <VStack spacing={4}>
                          ...
                          >
                            <Icon as={BsEmojiSmile} fontSize="2xl" color="white" />
                          </Box>
                          <VStack spacing={2}>
                            <Text
                              fontFamily="'General Sans', 'Inter', sans-serif"
                              ...
                              textAlign="center"
                            >
                              {isDragActive ? "Drop your portrait here" : "Click or drag to upload"}
                            </Text>
                            <Text
                              fontFamily="'General Sans', 'Inter', sans-serif"
                              fontSize="sm"
                ...
                >
                  <Box
                    ...
                    flexShrink={0}
                  >
                    <Icon as={BsEmojiSmile} fontSize="xl" color="white" />
                  </Box>
                  <Text px={{ base: 4, md: 6 }} py={{ base: 3, md: 4 }} flex="1" textAlign="left">
                    {isProcessing ? "ANIME-FYING..." : "ANIME-FY PHOTO"}
                  </Text>
                </Button>
              </VStack>
            </Box>
                    ...
                    >
                      <Center flexDirection="column" minH="400px" gap={4}>
                        <Box
                          ...
                          borderColor="rgba(0, 0, 0, 0.2)"
                        >
                          <Icon as={BsEmojiSmile} fontSize="2xl" color="rgba(0, 0, 0, 0.4)" />
                        </Box>
                        <VStack spacing={2}>
                          <Text
                            fontFamily="'General Sans', 'Inter', sans-serif"
                            ...
                            textAlign="center"
                          >
                            Anime portrait will appear here
                          </Text>
                          <Text
                            fontFamily="'General Sans', 'Inter', sans-serif"
                            fontSize="sm"
                            ...
                            textAlign="center"
                          >
                            Upload a portrait to convert it to anime style
                          </Text>
                        </VStack>
                      </Center>
                    </MotionBox>
            ...
            >
              ...
              <Box maxW="3xl" mx="auto">
                <Text
                  ...
                  opacity={0.9}
                >
                  Character designs, portraits, and scenery reimagined in anime style.
                </Text>
              </Box>
              <SimpleGrid columns={{ base: 1, md: 3 }} spacing={6} width="100%">
                {animeExamples.map((example, index) => (
                  <MotionBox
                    key={example.id}
                    borderRadius="16px"
                    overflow="hidden"
                  ...
                  >
                    <Image
                      ...
                      objectFit="cover"
                      borderRadius="16px"
                      fallbackSrc="https://via.placeholder.com/400x400/f0f0f0/999999?text=Anime+Example"
                    />
                  </MotionBox>
                ))}
              </SimpleGrid>
...
export default AIAnimeFilterPage;

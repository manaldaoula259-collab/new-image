...
const comicExamples = [
  {
    ...
    alt: "Manga style comic",
    label: "Manga Style",
  },
  {
    id: 2,
    src: "/prompts/romy/samurai.png",
    alt: "Ink drawing comic",
    label: "Ink Drawing",
  },
  {
    id: 3,
    src: "/prompts/romy/superhero.png",
    alt: "American comic style",
    label: "American Comic",
  },
  {
    id: 4,
    src: "/prompts/romy/cyberpunk.png",
    alt: "Modern comic style",
    label: "Modern Comic",
  },
];

const DEFAULT_RESULT_FILENAME = "ai-comic.jpg";
...
const AIComicGeneratorPage = () => {
  ...
  const handleGenerate = async () => {
    ...
    if (!prompt.trim()) {
      toast({
        ...
        description: "Describe what comic you want to generate.",
        status: "warning",
        duration: 3000,
        isClosable: true,
        position: "top-right",
    ...
    try {
      const response = await fetch("/api/ai-art-generator/ai-comic-generator", {
        method: "POST",
        headers: {
      ...
      if (!response.ok) {
        let message = "Comic generation failed.";
        try {
      ...
      if (!data?.resultUrl) {
        throw new Error("Comic generation did not return a result URL.");
      }
      ...
      toast({
        ...
        description: "Your comic illustration is ready to download.",
        status: "success",
        duration: 3000,
        isClosable: true,
        position: "top-right",
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error("Unable to download the comic.");
      }
      ...
      toast({
        ...
        description: "We opened the comic in a new tab so you can download it manually.",
        status: "info",
        duration: 3000,
        isClosable: true,
        position: "top-right",
  ...
  const handleSaveToLibrary = async () => {
    ...
    try {
      const response = await fetch("/api/media", {
        ...
        body: JSON.stringify({
          url: resultUrl,
          prompt: prompt || "AI Comic Generator",
          source: "ai-comic-generator",
        }),
      });

      if (!response.ok) {
        let message = "Failed to save comic to your library.";
        try {
          const errorPayload = await response.json();
          if (errorPayload?.error) {
            message = errorPayload.error;
      ...
      toast({
        ...
        description:
          error instanceof Error
            ? error.message
            : "Unable to save this comic. Please try again.",
        status: "error",
        duration: 4000,
        isClosable: true,
        position: "top-right",
  ...
  const handleExampleClick = (example: typeof comicExamples[0]) => {
          ...
          <VStack spacing={12} width="100%" align="stretch">
                ...
                <VStack spacing={4} position="relative" zIndex={1}>
                  ...
                  >
                    ...
                    <Button
                      ...
                      isDisabled={!isLoaded || !isSignedIn || !prompt.trim() || isGenerating}
                    ...
                    >
                      ...
                      <Text px={{ base: 0, md: 0 }} py={{ base: 3, md: 4 }} textAlign="left">
            ...
            >
              ...
              <SimpleGrid
                ...
                width="100%"
              >
                {comicExamples.map((example, index) => (
                  <MotionBox
                    key={example.id}
                    borderRadius="16px"
                    overflow="hidden"
                  ...
                  >
                    <Box position="relative" width="100%" height="100%">
                      <Image
                        ...
                        objectFit="cover"
                        borderRadius="16px"
                        fallbackSrc="https://via.placeholder.com/400x400/f0f0f0/999999?text=Comic+Example"
                      />
                      <Box
                        position="absolute"
                        inset={0}
...
export default AIComicGeneratorPage;

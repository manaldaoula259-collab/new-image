...
const DEFAULT_RESULT_FILENAME = "3d-render.jpg";
const DEFAULT_CONVERSION_PROMPT =
  "High quality 3D render, distinct depth layers, photorealistic, 8k";
...
// Example images for the bottom section
const example3DImages = [
  {
    id: 1,
    src: "/example-3d-character.png",
    alt: "Stylized character render",
    label: "Stylized Character",
    description: "Painterly warrior sculpted from a flat illustration.",
  },
  {
    id: 2,
    src: "/example-3d-product.png",
    alt: "Product hero render",
    label: "Product Hero",
    description: "Studio lighting and reflections built from a product shot.",
  },
  {
    id: 3,
    src: "/example-3d-portrait.png",
    alt: "Cinematic portrait render",
    label: "Portrait Depth",
    description: "Portrait with realistic depth, skin lighting, and bokeh.",
  },
  {
    id: 4,
    src: "/example-3d-environment.png",
    alt: "3D neon cityscape",
    label: "Cinematic Environment",
    description: "Hand-painted scenery converted into a dimensional world.",
  },
];

const Converter2DTo3DPage = () => {
  ...
  const [lastPrompt, setLastPrompt] = useState<string>(DEFAULT_CONVERSION_PROMPT);
  ...
  const { getRootProps, getInputProps, isDragActive } = useDropzone({
    ...
    onDrop: async (acceptedFiles) => {
      if (acceptedFiles?.[0]) {
        ...
        setLastPrompt(DEFAULT_CONVERSION_PROMPT);
      }
    },
    onDropRejected: () => {
      toast({
  ...
  const handleConvert = async () => {
    ...
    try {
      ...
      const response = await fetch("/api/ai-photo-filter/2d-to-3d-converter", {
        method: "POST",
        headers: {
      ...
      if (!response.ok) {
        let message = "The conversion service returned an unexpected error.";
        try {
      ...
      if (!data?.resultUrl) {
        throw new Error("The conversion service did not return a result URL.");
      }

      setResultUrl(data.resultUrl);
      setResultFileName(deriveFileNameFromUrl(data.resultUrl));
      setLastPrompt(data.prompt ?? DEFAULT_CONVERSION_PROMPT);

      toast({
        title: "Conversion complete!",
        description: "Your 3D model is ready for download.",
        status: "success",
        duration: 3000,
        isClosable: true,
        position: "top-right",
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error("Unable to download the generated 3D model.");
      }
      ...
      toast({
        ...
        description: "We opened the Replicate output in a new tab so you can download it manually.",
        status: "info",
        duration: 3000,
        isClosable: true,
        position: "top-right",
  ...
  const handleSaveToLibrary = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        let message = "Failed to save media to your library.";
        try {
          const errorPayload = await response.json();
          if (errorPayload?.error) {
            message = errorPayload.error;
      ...
      toast({
        ...
        description:
          error instanceof Error
            ? error.message
            : "Unable to save this result. Please try again.",
        status: "error",
        duration: 4000,
        isClosable: true,
        position: "top-right",
            ...
            >
              ...
              <Box maxW={{ base: "100%", md: "800px" }}>
                <Text
                  ...
                  opacity={0.9}
                >
                  Upload a flat image and let our AI build a depth-perfect 3D model in moments—no complex tools required.
                </Text>
              </Box>
            </MotionVStack>
                  ...
                  <VStack spacing={4} height="100%">
                        ...
                        >
                          ...
                          >
                            ...
                            <VStack spacing={4}>
                              ...
                              >
                                <Icon as={BsBoxSeam} fontSize="2xl" color="white" />
                              </Box>
                              <VStack spacing={2}>
                                <Text
                                  fontFamily="'General Sans', 'Inter', sans-serif"
                    ...
                    >
                      <Box
                        ...
                        flexShrink={0}
                      >
                        <Icon as={BsBoxSeam} fontSize="xl" color="white" />
                      </Box>
                      <Text px={{ base: 4, md: 6 }} py={{ base: 3, md: 4 }} flex="1" textAlign="left">
                        {isProcessing ? "CONVERTING..." : "CONVERT TO 3D"}
                      </Text>
                    </Button>
                  </VStack>
                </Box>
                        ...
                        >
                          <Center flexDirection="column" minH="400px" gap={4}>
                            <Box
                              ...
                              borderColor="rgba(0, 0, 0, 0.2)"
                            >
                              <Icon as={BsBoxSeam} fontSize="2xl" color="rgba(0, 0, 0, 0.4)" />
                            </Box>
                            <VStack spacing={2}>
                              <Text
                                fontFamily="'General Sans', 'Inter', sans-serif"
                              ...
                              <Text
                                ...
                                textAlign="center"
                              >
                                Upload an image to convert it to 3D
                              </Text>
                            </VStack>
                          </Center>
                        </MotionBox>
            ...
            >
              ...
              <Box maxW="3xl" mx="auto">
                <Text
                  ...
                  opacity={0.9}
                >
                  A quick look at the kinds of depth, lighting, and detail the converter can recreate from a single flat input.
                </Text>
              </Box>
              <SimpleGrid columns={{ base: 2, sm: 4 }} spacing={5} width="100%">
                {example3DImages.map((example, index) => (
                  <MotionBox
                    key={example.id}
                    borderRadius="16px"
                    overflow="hidden"
                  ...
                  >
                    <Box position="relative" width="100%" height="100%">
                      <Image
                        ...
                        objectFit="cover"
                        borderRadius="16px"
                        fallbackSrc="https://via.placeholder.com/400x400/f0f0f0/999999?text=3D+Example"
                      />
                      <Box
                        position="absolute"
                        inset={0}
                      ...
                      <Box
                        ...
                        fontFamily="'IBM Plex Mono', monospace"
                      >
                        {String(example.id).padStart(2, "0")}
                      </Box>
                      <VStack
                        align="flex-start"
                        spacing={1}
...
export default Converter2DTo3DPage;

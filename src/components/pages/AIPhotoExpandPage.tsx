...
const DEFAULT_RESULT_FILENAME = "ai-photo-expand.jpg";
...
const photoExpandExamples = [
  {
    id: 1,
    src: "/assets/ai-photo-expand/1.png",
    alt: "AI photo expand example 1",
  },
  {
    id: 2,
    src: "/assets/ai-photo-expand/2.png",
    alt: "AI photo expand example 2",
  },
  {
    id: 3,
    src: "/assets/ai-photo-expand/3.png",
    alt: "AI photo expand example 3",
  },
];

const AIPhotoExpandPage = () => {
  ...
  const handleConvert = async () => {
    ...
    if (!uploadedImage) {
      toast({
        ...
        description: "Upload an image to expand it.",
        status: "warning",
        duration: 3000,
        isClosable: true,
        position: "top-right",
    ...
    try {
      ...
      const response = await fetch("/api/ai-image-editor/ai-photo-expand", {
        method: "POST",
        headers: {
      ...
      if (!response.ok) {
        let message = "The photo expansion service returned an unexpected error.";
        try {
      ...
      if (!data?.resultUrl) {
        throw new Error("The photo expansion service did not return a result URL.");
      }
      ...
      toast({
        ...
        description: "Your expanded photo is ready to download.",
        status: "success",
        duration: 3000,
        isClosable: true,
        position: "top-right",
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error("Unable to download the expanded image.");
      }
      ...
      toast({
        ...
        description: "We opened the expanded image in a new tab so you can download it manually.",
        status: "info",
        duration: 3000,
        isClosable: true,
        position: "top-right",
  ...
  const handleSaveToLibrary = async () => {
    ...
    try {
      const response = await fetch("/api/media", {
        ...
        body: JSON.stringify({
          url: resultUrl,
          prompt: expandPrompt.trim() || "AI Photo Expand - Expanded image",
          source: "ai-photo-expand",
        }),
      });
      if (!response.ok) {
          ...
          <SimpleGrid columns={{ base: 1, lg: 2 }} spacing={6} width="100%">
            ...
            >
              ...
              >
                    ...
                    >
                      ...
                      >
                        ...
                        <VStack spacing={4}>
                          ...
                          >
                            <Icon as={BsArrowsFullscreen} fontSize="2xl" color="white" />
                          </Box>
                          <VStack spacing={2}>
                            <Text
                              fontFamily="'General Sans', 'Inter', sans-serif"
                ...
                <VStack spacing={3} width="100%" position="relative" zIndex={1}>
                  <Textarea
                    placeholder="Expansion instructions (optional) - e.g., Expand the background to show more sky..."
                    value={expandPrompt}
                  ...
                  >
                    <Box
                      ...
                      flexShrink={0}
                    >
                      <Icon as={BsArrowsFullscreen} fontSize="xl" color="white" />
                    </Box>
                    <Text px={{ base: 4, md: 6 }} py={{ base: 3, md: 4 }} flex="1" textAlign="left">
                      {isProcessing ? "EXPANDING..." : "EXPAND PHOTO"}
                    </Text>
                  </Button>
                </VStack>
              </VStack>
                    ...
                    >
                      <Center flexDirection="column" minH="400px" gap={4}>
                        <Box
                          ...
                          borderColor="rgba(0, 0, 0, 0.2)"
                        >
                          <Icon as={BsArrowsFullscreen} fontSize="2xl" color="rgba(0, 0, 0, 0.4)" />
                        </Box>
                        <VStack spacing={2}>
                          <Text
                            fontFamily="'General Sans', 'Inter', sans-serif"
                            ...
                            textAlign="center"
                          >
                            Expanded photo will appear here
                          </Text>
                          <Text
                            fontFamily="'General Sans', 'Inter', sans-serif"
                            fontSize="sm"
                            ...
                            textAlign="center"
                          >
                            Upload and expand an image to see the result
                          </Text>
                        </VStack>
                      </Center>
                    </MotionBox>
            ...
            >
              <Text
                ...
                letterSpacing="-0.6px"
              >
                Example Photo Expansion Results
              </Text>
              <Box maxW="3xl" mx="auto">
                <Text
                  ...
                  opacity={0.9}
                >
                  Expand your images beyond their borders with AI-powered content generation.
                </Text>
              </Box>
              <SimpleGrid columns={{ base: 1, md: 3 }} spacing={6} width="100%">
                {photoExpandExamples.map((example, index) => (
                  <MotionBox
                    key={example.id}
                    borderRadius="16px"
                    overflow="hidden"
...
export default AIPhotoExpandPage;

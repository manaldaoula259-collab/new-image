...
const DEFAULT_RESULT_FILENAME = "remove-object-from-image.jpg";
...
const removeObjectExamples = [
  {
    id: 1,
    src: "/assets/remove-object-from-image/1.png",
    alt: "Remove object from image example 1",
  },
  {
    id: 2,
    src: "/assets/remove-object-from-image/2.png",
    alt: "Remove object from image example 2",
  },
  {
    id: 3,
    src: "/assets/remove-object-from-image/3.png",
    alt: "Remove object from image example 3",
  },
];

const RemoveObjectFromImagePage = () => {
  ...
  const handleConvert = async () => {
    ...
    if (!uploadedImage) {
      toast({
        ...
        description: "Upload an image to remove objects from it.",
        status: "warning",
        duration: 3000,
        isClosable: true,
        position: "top-right",
    ...
    if (!objectDescription.trim()) {
      toast({
        ...
        description: "Please describe what object you want to remove from the image.",
        status: "warning",
        duration: 3000,
        isClosable: true,
        position: "top-right",
    ...
    try {
      ...
      const response = await fetch("/api/ai-image-editor/remove-object-from-image", {
        method: "POST",
        headers: {
      ...
      if (!response.ok) {
        let message = "The object removal service returned an unexpected error.";
        try {
      ...
      if (!data?.resultUrl) {
        throw new Error("The object removal service did not return a result URL.");
      }
  ...
  const handleSaveToLibrary = async () => {
    ...
    try {
      const response = await fetch("/api/media", {
        ...
        body: JSON.stringify({
          url: resultUrl,
          prompt: lastPrompt || "Remove object from image",
          source: "remove-object-from-image",
        }),
      });
      ...
      toast({
        ...
        description:
          error instanceof Error
            ? error.message
            : "Unable to save this image. Please try again.",
        status: "error",
        duration: 4000,
        isClosable: true,
        position: "top-right",
            ...
            >
              ...
              <Box maxW={{ base: "100%", md: "800px" }}>
                <Text
                  ...
                  opacity={0.9}
                >
                  Effortlessly tweak and remove unwanted objects from your images with AI-powered editing. Designed for all skill levels to unlock creative potential in seconds!
                </Text>
              </Box>
            </MotionVStack>
            ...
            >
              ...
              >
                    ...
                    >
                      ...
                      >
                        ...
                        <VStack spacing={4}>
                          ...
                          >
                            <Icon as={BsDashCircle} fontSize="2xl" color="white" />
                          </Box>
                          <VStack spacing={2}>
                            <Text
                              fontFamily="'General Sans', 'Inter', sans-serif"
                ...
                <VStack spacing={3} width="100%" position="relative" zIndex={1}>
                  <Textarea
                    placeholder="Describe the object to remove (e.g., the person in the background, the car on the street, the sign on the wall...)"
                    ...
                    _hover={{ borderColor: "rgba(0, 0, 0, 0.3)" }}
                    minH="100px"
                    resize="vertical"
                    fontSize="sm"
                    fontFamily="'General Sans', 'Inter', sans-serif"
                  ...
                  >
                    <Box
                      ...
                      flexShrink={0}
                    >
                      <Icon as={BsDashCircle} fontSize="xl" color="white" />
                    </Box>
                    <Text px={{ base: 4, md: 6 }} py={{ base: 3, md: 4 }} flex="1" textAlign="left">
                      {isProcessing ? "REMOVING OBJECT..." : "REMOVE OBJECT FROM IMAGE"}
                    </Text>
                  </Button>
                </VStack>
              </VStack>
                    ...
                    >
                      <Center flexDirection="column" minH="400px" gap={4}>
                        <Box
                          ...
                          borderColor="rgba(0, 0, 0, 0.2)"
                        >
                          <Icon as={BsDashCircle} fontSize="2xl" color="rgba(0, 0, 0, 0.4)" />
                        </Box>
                        <VStack spacing={2}>
                          <Text
                            fontFamily="'General Sans', 'Inter', sans-serif"
            ...
            >
              ...
              <Box maxW="3xl" mx="auto">
                <Text
                  ...
                  opacity={0.9}
                >
                  Cleanly remove unwanted objects, people, and distractions from your photos.
                </Text>
              </Box>
              <SimpleGrid columns={{ base: 1, md: 3 }} spacing={6} width="100%">
                {removeObjectExamples.map((example, index) => (
                  <MotionBox
                    key={example.id}
                    borderRadius="16px"
                    overflow="hidden"
...
export default RemoveObjectFromImagePage;

...
const DEFAULT_RESULT_FILENAME = "vectorize-image.png";
...
const vectorizeExamples = [
  {
    id: 1,
    src: "/assets/vectorize-image/1.png",
    alt: "Vectorize image example 1",
  },
  {
    id: 2,
    src: "/assets/vectorize-image/2.png",
    alt: "Vectorize image example 2",
  },
  {
    id: 3,
    src: "/assets/vectorize-image/3.png",
    alt: "Vectorize image example 3",
  },
];

const VectorizeImagePage = () => {
  ...
  const handleConvert = async () => {
    ...
    if (!uploadedImage) {
      toast({
        ...
        description: "Upload an image to vectorize it.",
        status: "warning",
        duration: 3000,
        isClosable: true,
        position: "top-right",
    ...
    try {
      ...
      const response = await fetch("/api/ai-image-editor/vectorize-image", {
        method: "POST",
        headers: {
      ...
      if (!response.ok) {
        let message = "The vectorization service returned an unexpected error.";
        try {
      ...
      if (!data?.resultUrl) {
        throw new Error("The vectorization service did not return a result URL.");
      }
      ...
      toast({
        ...
        description: "Your vectorized image is ready to download.",
        status: "success",
        duration: 3000,
        isClosable: true,
        position: "top-right",
      ...
      toast({
        title: "Image vectorization failed",
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error("Unable to download the vectorized image.");
      }
      ...
      toast({
        ...
        description: "We opened the vectorized image in a new tab so you can download it manually.",
        status: "info",
        duration: 3000,
        isClosable: true,
        position: "top-right",
  ...
  const handleSaveToLibrary = async () => {
    ...
    try {
      const response = await fetch("/api/media", {
        ...
        body: JSON.stringify({
          url: resultUrl,
          prompt: "Vectorize Image - Converted to vector style",
          source: "vectorize-image",
        }),
      });
      if (!response.ok) {
              ...
              >
                    ...
                    >
                      ...
                      >
                        ...
                        <VStack spacing={4}>
                          ...
                          >
                            <Icon as={BsVectorPen} fontSize="2xl" color="white" />
                          </Box>
                          <VStack spacing={2}>
                            <Text
                              fontFamily="'General Sans', 'Inter', sans-serif"
                ...
                >
                  <Box
                    ...
                    flexShrink={0}
                  >
                    <Icon as={BsVectorPen} fontSize="xl" color="white" />
                  </Box>
                    ...
                    >
                      <Center flexDirection="column" minH="400px" gap={4}>
                        <Box
                          ...
                          borderColor="rgba(0, 0, 0, 0.2)"
                        >
                          <Icon as={BsVectorPen} fontSize="2xl" color="rgba(0, 0, 0, 0.4)" />
                        </Box>
                        <VStack spacing={2}>
                          <Text
                            fontFamily="'General Sans', 'Inter', sans-serif"
                            ...
                            textAlign="center"
                          >
                            Vectorized image will appear here
                          </Text>
                          <Text
                            fontFamily="'General Sans', 'Inter', sans-serif"
                            fontSize="sm"
                            ...
                            textAlign="center"
                          >
                            Upload and vectorize an image to see the result
                          </Text>
                        </VStack>
                      </Center>
                    </MotionBox>
            ...
            >
              ...
              <Box maxW="3xl" mx="auto">
                <Text
                  ...
                  opacity={0.9}
                >
                  Convert raster images to scalable vector graphics with precision and quality.
                </Text>
              </Box>
              <SimpleGrid columns={{ base: 1, md: 3 }} spacing={6} width="100%">
                {vectorizeExamples.map((example, index) => (
                  <MotionBox
                    key={example.id}
                    borderRadius="16px"
                    overflow="hidden"
...
export default VectorizeImagePage;

...
const animeExamples = [
  {
    ...
    alt: "Kawaii anime character",
    label: "Kawaii Style",
  },
  {
    id: 2,
    src: "/prompts/romy/cyberpunk.png",
    alt: "Cyberpunk anime character",
    label: "Cyberpunk",
  },
  {
    id: 3,
    src: "/prompts/romy/superhero.png",
    alt: "Superhero anime character",
    label: "Superhero",
  },
  {
    id: 4,
    src: "/prompts/romy/elf.png",
    alt: "Elf anime character",
    label: "Fantasy Elf",
  },
];

const DEFAULT_RESULT_FILENAME = "ai-anime.jpg";
...
const AIAnimeGeneratorPage = () => {
  ...
  const handleGenerate = async () => {
    ...
    if (!prompt.trim()) {
      toast({
        ...
        description: "Describe what anime character you want to generate.",
        status: "warning",
        duration: 3000,
        isClosable: true,
        position: "top-right",
    ...
    try {
      const response = await fetch("/api/ai-art-generator/ai-anime-generator", {
        method: "POST",
        headers: {
      ...
      if (!response.ok) {
        let message = "Anime generation failed.";
        try {
      ...
      if (!data?.resultUrl) {
        throw new Error("Anime generation did not return a result URL.");
      }
      ...
      toast({
        title: "Anime art generated!",
        description: "Your anime illustration is ready to download.",
        status: "success",
        duration: 3000,
        isClosable: true,
        position: "top-right",
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error("Unable to download the anime art.");
      }
      ...
      toast({
        ...
        description: "We opened the anime art in a new tab so you can download it manually.",
        status: "info",
        duration: 3000,
        isClosable: true,
        position: "top-right",
  ...
  const handleSaveToLibrary = async () => {
    ...
    try {
      const response = await fetch("/api/media", {
        ...
        body: JSON.stringify({
          url: resultUrl,
          prompt: prompt || "AI Anime Generator",
          source: "ai-anime-generator",
        }),
      });

      if (!response.ok) {
        let message = "Failed to save anime art to your library.";
        try {
          const errorPayload = await response.json();
          if (errorPayload?.error) {
            message = errorPayload.error;
      ...
      toast({
        ...
        description:
          error instanceof Error
            ? error.message
            : "Unable to save this anime art. Please try again.",
        status: "error",
        duration: 4000,
        isClosable: true,
        position: "top-right",
  ...
  const handleExampleClick = (example: typeof animeExamples[0]) => {
...
export default AIAnimeGeneratorPage;

...
const DEFAULT_RESULT_FILENAME = "ai-style-transfer.jpg";
...
const styleOptions = [
  { value: "painterly", label: "Painterly", description: "Brush strokes & textures" },
  { value: "cyberpunk", label: "Cyberpunk", description: "Neon & futuristic" },
  { value: "abstract", label: "Abstract", description: "Geometric & creative" },
  { value: "watercolor", label: "Watercolor", description: "Soft & flowing" },
  { value: "oil_painting", label: "Oil Painting", description: "Classic & textured" },
  { value: "sketch", label: "Sketch", description: "Pencil & detailed" },
];

const styleExamples = [
  {
    id: 1,
    src: "/style-transfer-impressionist.jpeg",
    alt: "Impressionist painting style",
  },
  {
    id: 2,
    src: "/style-transfer-cyberpunk.jpeg",
    alt: "Cyberpunk neon style",
  },
  {
    id: 3,
    src: "/style-transfer-watercolor.jpeg",
    alt: "Soft watercolor style",
  },
];

const AIStyleTransferPage = () => {
  const [uploadedImage, setUploadedImage] = useState<FilePreview | null>(null);
  const [isProcessing, setIsProcessing] = useState(false);
  const [resultUrl, setResultUrl] = useState<string | null>(null);
  const [selectedStyle, setSelectedStyle] = useState<string>("painterly");
  const [resultFileName, setResultFileName] = useState(DEFAULT_RESULT_FILENAME);
  const [isDownloading, setIsDownloading] = useState(false);
  const [isSavingToLibrary, setIsSavingToLibrary] = useState(false);
  const [isSavedToLibrary, setIsSavedToLibrary] = useState(false);
  const [lastStyle, setLastStyle] = useState<string>("painterly");
  ...
  const handleConvert = async () => {
    ...
    if (!uploadedImage) {
      toast({
        ...
        description: "Upload an image to apply a new artistic style.",
        status: "warning",
        duration: 3000,
        isClosable: true,
        position: "top-right",
    ...
    try {
      ...
      const response = await fetch("/api/ai-photo-filter/ai-style-transfer", {
        method: "POST",
        headers: {
      ...
      if (!response.ok) {
        let message = "The style transfer service returned an unexpected error.";
        try {
      ...
      if (!data?.resultUrl) {
        throw new Error("The style transfer service did not return a result URL.");
      }
      ...
      toast({
        title: "Style applied!",
        description: "Download the styled artwork or try another look.",
        status: "success",
        duration: 3000,
        isClosable: true,
        position: "top-right",
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error("Unable to download the styled artwork.");
      }
      ...
      toast({
        ...
        description: "We opened the styled artwork in a new tab so you can download it manually.",
        status: "info",
        duration: 3000,
        isClosable: true,
        position: "top-right",
  ...
  const handleSaveToLibrary = async () => {
    ...
    try {
      const response = await fetch("/api/media", {
        ...
        body: JSON.stringify({
          url: resultUrl,
          prompt: `AI Style Transfer - ${lastStyle} style`,
          source: "ai-style-transfer",
        }),
      });

      if (!response.ok) {
        let message = "Failed to save artwork to your library.";
        try {
          const errorPayload = await response.json();
          if (errorPayload?.error) {
            message = errorPayload.error;
      ...
      toast({
        ...
        description:
          error instanceof Error
            ? error.message
            : "Unable to save this artwork. Please try again.",
        status: "error",
        duration: 4000,
        isClosable: true,
        position: "top-right",
            ...
            >
              ...
              <Box maxW={{ base: "100%", md: "800px" }}>
                <Text
                  ...
                  opacity={0.9}
                >
                  Transform any photo into a work of art in seconds—try painterly brush strokes, neon cyberpunk, abstract shapes, and more with AI-powered style transfer.
                </Text>
              </Box>
            </MotionVStack>
          ...
          <SimpleGrid columns={{ base: 1, lg: 2 }} spacing={6} width="100%">
            ...
            >
              ...
              >
                    ...
                    >
                      ...
                      >
                        ...
                        <VStack spacing={3}>
                          ...
                          >
                            <Icon as={BsBrush} fontSize="xl" color="white" />
                          </Box>
                          <VStack spacing={1}>
                            <Text
                              fontFamily="'General Sans', 'Inter', sans-serif"
                ...
                <VStack spacing={3} width="100%" position="relative" zIndex={1}>
                  <Text
                    fontFamily="'General Sans', 'Inter', sans-serif"
                    fontSize="sm"
                    fontWeight="600"
                  ...
                  <SimpleGrid columns={3} spacing={2} width="100%">
                    {styleOptions.map((style) => (
                      <Button
                        ...
                        bg={selectedStyle === style.value ? "#573cff" : "white"}
                        borderColor={selectedStyle === style.value ? "transparent" : "rgba(0, 0, 0, 0.2)"}
                        color={selectedStyle === style.value ? "white" : "black"}
                        _hover={{
                          bg: selectedStyle === style.value ? "#4c2fd6" : "rgba(0, 0, 0, 0.05)",
                          borderColor: selectedStyle === style.value ? "transparent" : "rgba(0, 0, 0, 0.3)",
                        }}
                        fontSize="xs"
                        py={2}
                        px={2}
                      ...
                      >
                        <VStack spacing={0}>
                          <Text fontWeight="700">{style.label}</Text>
                    ...
                    >
                      <Center flexDirection="column" minH="400px" gap={4}>
                        <Box
                          ...
                          borderColor="rgba(0, 0, 0, 0.2)"
                        >
                          <Icon as={BsBrush} fontSize="2xl" color="rgba(0, 0, 0, 0.4)" />
                        </Box>
                        <VStack spacing={2}>
                          <Text
                            fontFamily="'General Sans', 'Inter', sans-serif"
                            ...
                            textAlign="center"
                          >
                            Styled artwork will appear here
                          </Text>
                          <Text
                            fontFamily="'General Sans', 'Inter', sans-serif"
                            fontSize="sm"
            ...
            {/* Example Style Transformations Section */}
            <MotionVStack
              spacing={6}
              width="100%"
              pt={8}
            ...
            >
              ...
              <Box maxW="3xl" mx="auto">
                <Text
                  ...
                  opacity={0.9}
                >
                  A glimpse at the creative palette—mix painterly strokes, neon glows, dreamy watercolors, and more.
                </Text>
              </Box>
                ...
                {styleExamples.map((example, index) => (
                  ...
                  >
                    <Image
                      ...
                      objectFit="cover"
                      borderRadius="16px"
                      fallbackSrc="https://via.placeholder.com/400x400/f0f0f0/999999?text=Style+Example"
                    />
                  </MotionBox>
                ))}
              </SimpleGrid>
...
export default AIStyleTransferPage;

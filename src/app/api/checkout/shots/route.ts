...
export async function GET(req: Request) {
  ...
  try {
    const session = await stripe.checkout.sessions.create({
      ...
      mode: "payment",
      success_url: `${process.env.NEXT_PUBLIC_APP_URL ?? process.env.NEXTAUTH_URL}/studio/${ppi}/?session_id={CHECKOUT_SESSION_ID}&ppi=${ppi}`,
      cancel_url: `${process.env.NEXT_PUBLIC_APP_URL ?? process.env.NEXTAUTH_URL}/studio/${ppi}`,
    });

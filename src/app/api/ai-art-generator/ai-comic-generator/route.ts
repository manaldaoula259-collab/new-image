...
export async function POST(request: NextRequest) {
  ...
  const resolved = await resolveReplicateModelIdentifierForSlug("ai-comic-generator");
  const modelIdentifier = (resolved.identifier && resolved.confidence >= 30)
    ? resolved.identifier
    : FALLBACK_MODEL_IDENTIFIER;

  const enhancedPrompt = `Create a comic book style illustration: ${prompt.trim()}. Bold comic book art style, vibrant comic colors, dynamic comic book illustration, professional comic book artwork, comic book panel style, comic book character design, high-quality comic art. Perfect for comic books, graphic novels, and comic-style artwork. No text, no watermarks, clean comic illustration.`;
  ...
  try {
    logger.info("[api/ai-comic-generator] Calling Replicate", {
    ...
    logger.info("[api/ai-comic-generator] Comic generation successful", {
    ...
    try {
      ...
      if (userId) {
        logger.info("[ai-comic-generator] Saving media to library", { userId, resultUrl });
        await saveGeneratedMedia({
          userId,
          url: resultUrl,
          prompt: typeof prompt === "string" ? prompt : enhancedPrompt,
          source: "tool:ai-art-generator/ai-comic-generator",
        });
        logger.info("[ai-comic-generator] Media saved successfully", { userId });
      } else {
        logger.warn("[ai-comic-generator] No userId found, skipping media save");
      }
    } catch (e) {
      logger.apiError("[ai-comic-generator] Failed to save media", e, { resultUrl });
    }
    ...
    logger.apiError("/api/ai-comic-generator", error instanceof Error ? error : new Error(String(error)), {
      prompt: prompt?.toString().substring(0, 100),
    });
    return NextResponse.json(
      {
        error: "Failed to generate comic art.",

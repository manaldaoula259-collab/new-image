...
export async function POST(request: NextRequest) {
  ...
  const resolved = await resolveReplicateModelIdentifierForSlug("ai-character-generator");
  const modelIdentifier = (resolved.identifier && resolved.confidence >= 30)
    ? resolved.identifier
    : FALLBACK_MODEL_IDENTIFIER;

  const enhancedPrompt = `Create a unique character: ${prompt.trim()}. High-quality character design, detailed features, professional illustration, vibrant colors, clean artwork. Perfect character design for games, stories, or creative projects. No text, no watermarks, clean character illustration.`;
  ...
  try {
    logger.info("[api/ai-character-generator] Calling Replicate", {
    ...
    logger.info("[api/ai-character-generator] Character generation successful", {
    ...
    try {
      ...
      if (userId) {
        logger.info("[ai-character-generator] Saving media to library", { userId, resultUrl });
        await saveGeneratedMedia({
          userId,
          url: resultUrl,
          prompt: typeof prompt === "string" ? prompt : enhancedPrompt,
          source: "tool:ai-art-generator/ai-character-generator",
        });
        logger.info("[ai-character-generator] Media saved successfully", { userId });
      } else {
        logger.warn("[ai-character-generator] No userId found, skipping media save");
      }
    } catch (e) {
      logger.apiError("[ai-character-generator] Failed to save media", e, { resultUrl });
    }
    ...
    logger.apiError("/api/ai-character-generator", error instanceof Error ? error : new Error(String(error)), {

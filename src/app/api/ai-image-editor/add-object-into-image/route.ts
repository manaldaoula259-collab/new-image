...
// Feature-specific prompt for adding objects
const SYSTEM_PROMPT = "You are an object addition engine using the Nano Banana model. The user will upload an image and specify an object to add. Add only the specified object naturally and seamlessly into the scene. Do not modify colors, lighting, perspective, or other elements unless necessary for realistic integration.";
...
export async function POST(request: NextRequest) {
  ...
  if (typeof prompt !== "string" || prompt.trim().length === 0) {
    return NextResponse.json(
      { error: "prompt is required and must be a non-empty string describing the object to add." },
      { status: 400 }
    );
  }
  ...
  try {
    ...
    if (!imageCheckResponse.ok) {
      logger.warn("[api/add-object-into-image] Image URL may not be accessible", {
        url: imageUrl.trim(),
        status: imageCheckResponse.status,
      });
    }
  } catch (error) {
    logger.warn("[api/add-object-into-image] Could not verify image URL accessibility", {
      url: imageUrl.trim(),
  ...
  // User's instruction is the primary directive
  const resolvedPrompt = `${SYSTEM_PROMPT} User instruction: ${prompt.trim()}. Apply only this instruction. Maintain realism and natural integration.`;

  // Build input with nano-banana parameters
  // Based on the turn-sketch-to-art example, nano-banana uses image_input as array
  ...
  try {
    logger.info("[api/add-object-into-image] Calling Replicate", {
    ...
    logger.info("[api/add-object-into-image] Object insertion successful", {
    ...
    logger.apiError("/api/add-object-into-image", error instanceof Error ? error : new Error(String(error)), {
      prompt: prompt?.toString().substring(0, 100),
    });
    return NextResponse.json(
      {
        error: "Failed to add object into image.",

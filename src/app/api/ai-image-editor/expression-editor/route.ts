...
const FEATURE_PROMPT = `You are a facial expression editing engine using the Nano Banana model.
...
export async function POST(request: NextRequest) {
  ...
  if (!userInstruction) {
    return NextResponse.json(
      { error: "prompt is required and must describe the desired expression (e.g., 'smile', 'serious', 'happy', 'surprised')." },
      { status: 400 }
    );
  }
  ...
  // Construct prompt: use feature prompt with user's expression instruction
  const resolvedPrompt = `${FEATURE_PROMPT} User instruction: ${userInstruction}. Apply only the expression change specified.`;
  ...
  try {
    logger.info("[api/expression-editor] Calling Replicate", {
    ...
    logger.info("[api/expression-editor] Expression edit successful", {
    ...
    try {
      ...
      if (userId) {
        await saveGeneratedMedia({
          ...
          url: resultUrl,
          prompt: resolvedPrompt,
          source: "tool:ai-image-editor/expression-editor",
        });
      }
    } catch (e) {
      logger.apiError("[expression-editor] Failed to save media", e);
    }
    ...
    logger.apiError("/api/expression-editor", error instanceof Error ? error : new Error(String(error)), {
      imageUrl: imageUrl.substring(0, 100),
    });
    return NextResponse.json(
      {
        error: "Failed to edit expression.",

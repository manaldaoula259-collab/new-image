...
// Using AnimateDiff Lightning for video generation - fast animated GIF and video generation model available on Replicate
const ANIMATEDIFF_LIGHTNING_MODEL = "toanbarcelona1998/animatediff-lightning-4-step_gif:66ac0a644a49d11e0acb9069310f6e0ce248791a5dfea39004610b9abe204f35";
...
export async function POST(request: NextRequest) {
  ...
  const type = typeof body.type === "string" ? body.type : "gif";
  const guidanceScale = typeof body.guidanceScale === "number" ? body.guidanceScale : 1;
  ...
  try {
    logger.info("[api/animatediff-lightning] Starting video generation with AnimateDiff Lightning", {
      promptLength: prompt.length,
      type,
      guidanceScale,
    });

    // Prepare input for AnimateDiff Lightning model
    // AnimateDiff Lightning supports: prompt, type, guidance_scale, negative_prompt
    ...
    // Run the video generation with AnimateDiff Lightning
    const output = await replicate.run(
      ANIMATEDIFF_LIGHTNING_MODEL as `${string}/${string}`,
      {
    ...
    logger.info("[api/animatediff-lightning] Video generation successful with AnimateDiff Lightning", {
    ...
    try {
      ...
      if (userId) {
        logger.info("[api/animatediff-lightning] Saving video to library", { userId, resultUrl });
        await saveGeneratedMedia({
          userId,
          url: resultUrl,
          prompt,
          source: "animatediff-lightning",
        });
        logger.info("[api/animatediff-lightning] Video saved successfully", { userId });
      }
    } catch (e) {
      logger.apiError("[api/animatediff-lightning] Failed to save video", e, { resultUrl });
    }

    return NextResponse.json({
      resultUrl,
      prompt,
      model: ANIMATEDIFF_LIGHTNING_MODEL,
      creditsDeducted: 1,
    });
  } catch (error) {
    logger.apiError(
      "/api/ai-video/animatediff-lightning",
    ...
    if (error instanceof Error) {
      if (error.message.includes("404") || error.message.includes("not found")) {
        return NextResponse.json(
          {
            error: "AnimateDiff Lightning model not found. Please check if the model is available on Replicate.",

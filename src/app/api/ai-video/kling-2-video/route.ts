...
// Using Kling V2.0 for video generation - high-quality video model available on Replicate
const KLING_V2_MODEL = "kwaivgi/kling-v2.0";
...
export async function POST(request: NextRequest) {
  ...
  const requestedAspectRatio = typeof body.aspectRatio === "string" ? body.aspectRatio : "16:9";
  ...
  // Kling V2.0 supports various aspect ratios, but we'll normalize to common formats
  // Default to "16:9" if not specified or invalid
  ...
  try {
    logger.info("[api/kling-2-video] Starting video generation with Kling V2.0", {
    ...
    // Prepare input for Kling V2.0 video model
    // Kling V2.0 supports: prompt, duration, cfg_scale, aspect_ratio, negative_prompt
    ...
    // Run the video generation with Kling V2.0
    const output = await replicate.run(KLING_V2_MODEL as `${string}/${string}`, {
      input: replicateInput,
    });

    // Handle video output (usually returns a URL or array of URLs)
    ...
    logger.info("[api/kling-2-video] Video generation successful with Kling V2.0", {
    ...
    try {
      ...
      if (userId) {
        logger.info("[api/kling-2-video] Saving video to library", { userId, resultUrl });
        await saveGeneratedMedia({
          userId,
          url: resultUrl,
          prompt,
          source: "kling-2-video",
        });
        logger.info("[api/kling-2-video] Video saved successfully", { userId });
      }
    } catch (e) {
      logger.apiError("[api/kling-2-video] Failed to save video", e, { resultUrl });
    }

    return NextResponse.json({
      resultUrl,
      prompt,
      model: KLING_V2_MODEL,
      creditsDeducted: 1,
    });
  } catch (error) {
    logger.apiError(
      "/api/ai-video/kling-2-video",
    ...
    if (error instanceof Error) {
      if (error.message.includes("404") || error.message.includes("not found")) {
        return NextResponse.json(
          {
            error: "Kling V2.0 video model not found. Please check if the model is available on Replicate.",
